...
export function getUserReview(reviewState: Record<string, UserReview>, slug: string): UserReview {
	return reviewState[slug] ?? DEFAULT_REVIEW_BY_SLUG.get(slug) ?? { decision: 'unverified', rejectionNote: undefined, personalTags: [] };
}
...
function normalizeUserReview(value: unknown): UserReview {
	...
	if (review.decision === 'approved' || review.decision === 'rejected' || review.decision === 'unverified') {
		return {
	...
	const decision: DecisionState = rejected ? 'rejected' : approved ? 'approved' : 'unverified';
